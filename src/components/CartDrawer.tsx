          ...
          <div className="p-5 border-b border-white/10 flex items-center justify-between bg-zinc-950/40 backdrop-blur-md shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                ...
                className="p-1.5 rounded-lg text-zinc-400 hover:text-white bg-zinc-900 border border-white/10 transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-3.5 divide-y divide-white/5">
            {items.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-white/10 flex items-center justify-center mb-4 text-zinc-500">
                  <ShoppingBag className="w-8 h-8" />
                </div>
                <p className="font-display font-bold text-white text-base">Your basket is empty</p>
                <p className="text-xs text-zinc-500 mt-1 max-w-xs leading-relaxed">
                  Browse our FiveM scripts catalogue and add resources to your order.
                </p>
                <button
                  onClick={handleClose}
                  className="mt-6 px-4 py-2 bg-white text-black font-semibold text-xs rounded-xl hover:bg-zinc-200 transition-colors"
                >
                  Explore Scripts
                </button>
              </div>
              ...
              <div key={`${item.package.id}-${idx}`} className="p-3.5 rounded-2xl bg-zinc-900/60 border border-white/10 flex gap-3.5 items-center hover:border-white/20 transition-all">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center mt-2 border border-white/10 rounded-lg bg-zinc-950 w-fit">
                    <button 
                      onClick={() => updateQuantity(item.package.id, -1)} 
                      aria-label={`Decrease quantity of ${item.package.name}`}
                      className="p-1 text-zinc-400 hover:text-white transition-colors px-2"
                    >
                ...
                <button 
                  ...
                  aria-label={`Remove ${item.package.name} from basket`}
