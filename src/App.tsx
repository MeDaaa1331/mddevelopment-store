...
const PaymentSuccessScreen: React.FC = () => {
  const { user, refreshPoints, showPointToast } = useAuth();
  ...
  useEffect(() => {
    ...
    const basketId = p.get('basketId') || p.get('ident') || p.get('basket') || p.get('tebex_basket_id');
    const targetUserId = p.get('userId') || user?.id;
    ...
    const verifyAndClaim = async () => {
      ...
      try {
        ...
        if (res.ok && data.success && data.pointsAwarded > 0) {
          const awarded = Number(data.pointsAwarded);
        ...
        console.warn('[PaymentSuccessScreen] Verification error:', err);
      } finally {
        setIsProcessing(false);
        refreshPoints();
      }
    ...
    verifyAndClaim();
  }, [user?.id, refreshPoints, showPointToast]);
    ...
    <div className="min-h-screen bg-[#050507] text-white flex flex-col items-center justify-center p-6 text-center font-sans">
      ...
      {isProcessing ? (
        <div className="mb-6 p-3.5 rounded-2xl bg-zinc-900/60 border border-white/10 flex items-center gap-3 text-left">
          ...
          <span className="text-xs text-zinc-300">Verifying and crediting your MD Points...</span>
        </div>
